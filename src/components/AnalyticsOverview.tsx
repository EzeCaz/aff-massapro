...
import { Loader2, Eye, Target, FileText, Phone, Users, TrendingUp, TrendingDown, MousePointerClick, FormInput, CalendarIcon, Filter, X, RotateCcw, ChevronDown, ChevronUp, Plus, Layers } from 'lucide-react'
import {
...
interface AdminStats {
  ...
  seriesBy?: string
  seriesData?: Record<string, DailyTimeSeries>
}
...
const SERIES_COLORS = [
  '#9333ea', '#2563eb', '#16a34a', '#d97706', '#dc2626',
...
function SparklineTooltip({ active, payload, label, metricLabel, metricUnit, seriesBy, seriesLabels }: any) {
  if (!active || !payload || !payload.length) return null
  const formattedDate = payload[0]?.payload?.date ? format(parseISO(payload[0].payload.date), 'MMM dd, yyyy') : label
  return (
    <div className="bg-white/95 backdrop-blur-sm border border-purple-200 rounded-lg px-3 py-2 shadow-lg shadow-purple-100/50 max-w-[200px]">
      <div className="text-[10px] font-semibold text-purple-700 mb-0.5">{metricLabel}</div>
      <div className="text-[10px] text-gray-500 mb-1">{formattedDate}</div>
      {payload.map((entry: any, idx: number) => (
        <div key={idx} className="flex items-center gap-1.5 text-xs">
          <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: entry.color || entry.stroke }} />
          <span className="text-gray-600">{seriesLabels?.[entry.dataKey] || entry.dataKey}:</span>
...
export default function AnalyticsOverview() {
  ...
  // Build multi-series data for sparklines
  const seriesGroups = useMemo(() => {
    if (!filters.seriesBy || !stats?.seriesData) return []
    return Object.keys(stats.seriesData).sort()
  }, [filters.seriesBy, stats?.seriesData])

  const seriesLabels = useMemo(() => {
    const labels: Record<string, string> = { value: 'Total' }
    seriesGroups.forEach((g, i) => {
      labels[`series_${i}`] = g === '(direct)' ? 'Direct' : g
    })
    return labels
  }, [seriesGroups])

  // Build merged chart data for each metric
  const buildMultiSeriesData = useCallback((metricKey: keyof DailyTimeSeries) => {
    const baseData = stats?.dailyTimeSeries?.[metricKey] || []
    if (!filters.seriesBy || seriesGroups.length === 0) return baseData

    return baseData.map((item, dayIdx) => {
      const point: Record<string, any> = { date: item.date, value: item.value }
      seriesGroups.forEach((group, gIdx) => {
        const groupSeries = stats?.seriesData?.[group]?.[metricKey]
        if (groupSeries && groupSeries[dayIdx]) {
          point[`series_${gIdx}`] = groupSeries[dayIdx].value
        }
      })
      return point
    })
  }, [stats?.dailyTimeSeries, stats?.seriesData, filters.seriesBy, seriesGroups])
  ...
  const hasSeries = filters.seriesBy && seriesGroups.length > 0
  ...
  const kpis = [
    {
      ...
      sparkColor: '#9333ea',
      sparkUnit: 'views',
      metricKey: 'traffic' as keyof DailyTimeSeries,
    },
    {
      ...
      sparkColor: '#4f46e5',
      sparkUnit: 'clicks',
      metricKey: 'ctaClicks' as keyof DailyTimeSeries,
    },
    {
      ...
      sparkColor: '#0891b2',
      sparkUnit: 'opens',
      metricKey: 'formOpens' as keyof DailyTimeSeries,
    },
    {
      ...
      sparkColor: '#2563eb',
      sparkUnit: 'leads',
      metricKey: 'referrals' as keyof DailyTimeSeries,
    },
    {
      ...
      sparkColor: '#16a34a',
      sparkUnit: '%',
      metricKey: 'conversionRate' as keyof DailyTimeSeries,
    },
    {
      label: 'Booking Rate',
      ...
      sparkColor: '#d97706',
      sparkUnit: '%',
      metricKey: 'bookingRate' as keyof DailyTimeSeries,
    },
    {
    ...
    <div className="space-y-6">
      ...
      <Card className="border-purple-200 shadow-sm">
        <CardContent className="p-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Graph Series Selector */}
            <div className="flex items-center gap-2 border-l border-purple-200 pl-3">
              <Layers className="w-3.5 h-3.5 text-purple-500" />
              <Label className="text-xs text-gray-500 whitespace-nowrap">Graph Series:</Label>
              <Select
                value={filters.seriesBy}
                onValueChange={v => setFilters(prev => ({ ...prev, seriesBy: v }))}
              >
                ...
                <SelectContent>
                  {SERIES_DIMENSIONS.map(d => (
                    <SelectItem key={d.value} value={d.value || '__none__'}>
                      {d.label}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          {hasSeries && (
            <div className="mt-3 pt-3 border-t border-purple-100">
              <div className="flex items-center gap-3 flex-wrap">
                <span className="text-xs font-medium text-gray-500">Series:</span>
                ...
                {seriesGroups.map((group, i) => (
                  <div key={group} className="flex items-center gap-1.5">
                    <span className="w-3 h-1.5 rounded-full" style={{ backgroundColor: SERIES_COLORS[i % SERIES_COLORS.length] }} />
                    <span className="text-xs text-gray-700">{group === '(direct)' ? 'Direct' : group}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4">
        {kpis.map(kpi => {
          ...
          const chartData = buildMultiSeriesData(kpi.metricKey)
          const hasChartData = chartData.length > 1

          return (
            <Card key={kpi.label} className="border-purple-100 hover:shadow-md transition-shadow">
              <CardContent className="p-4">
                ...
                {/* Sparkline Chart with Multi-Series Support */}
                {hasChartData && (
                  <div className="mt-2 h-12 cursor-crosshair">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={chartData} margin={{ top: 2, right: 2, bottom: 2, left: 2 }}>
                        <defs>
                          <linearGradient id={`sparkGrad-${kpi.label.replace(/[^a-zA-Z]/g, '')}`} x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor={kpi.sparkColor} stopOpacity={0.15} />
                            <stop offset="100%" stopColor={kpi.sparkColor} stopOpacity={0.02} />
                          </linearGradient>
                          {hasSeries && seriesGroups.map((_, i) => (
                            <linearGradient key={`sg-${i}`} id={`seriesGrad-${kpi.label.replace(/[^a-zA-Z]/g, '')}-${i}`} x1="0" y1="0" x2="0" y2="1">
                              <stop offset="0%" stopColor={SERIES_COLORS[i % SERIES_COLORS.length]} stopOpacity={0.1} />
                              <stop offset="100%" stopColor={SERIES_COLORS[i % SERIES_COLORS.length]} stopOpacity={0.01} />
                            </linearGradient>
                          ))}
                        </defs>
                        <Tooltip
                          content={<SparklineTooltip metricLabel={kpi.label} metricUnit={kpi.sparkUnit} seriesBy={filters.seriesBy} seriesLabels={seriesLabels} />}
                          cursor={{ stroke: '#e9d5ff', strokeWidth: 1, strokeDasharray: '3 3' }}
                        />
                        {/* Total line */}
                        <Area
                          type="monotone"
                          ...
                          strokeWidth={hasSeries ? 1.5 : 1.5}
                          fill={hasSeries ? 'none' : `url(#sparkGrad-${kpi.label.replace(/[^a-zA-Z]/g, '')})`}
                          dot={false}
                          activeDot={{
                            r: 3,
                            fill: kpi.sparkColor,
                        ...
                        />
                        {/* Series lines */}
                        {hasSeries && seriesGroups.map((group, i) => (
                          <Area
                            key={group}
                            type="monotone"
                            dataKey={`series_${i}`}
                            ...
                            strokeWidth={1.5}
                            fill={`url(#seriesGrad-${kpi.label.replace(/[^a-zA-Z]/g, '')}-${i})`}
                            dot={false}
                            activeDot={{
                              r: 2.5,
                            ...
                            isAnimationActive={false}
                            name={group === '(direct)' ? 'Direct' : group}
                          />
                        ))}
                      </AreaChart>
                    </ResponsiveContainer>
